...
export default function DonatePage() {
    ...
    async function handleSponsor() {
        ...
        await openRazorpayCheckout({
            ...
            onFailure: (msg) => {
                setSponsorResult({ success: false, error: msg });
                setSponsorLoading(false);
            },
        });
    }
            ...
            {tab === "sponsor" && (
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-5">
                    ...
                    <button
                        ...
                        aria-label="Pay and confirm sponsorship via Razorpay"
                    ...
                    >
                        {sponsorLoading ? (
                            ...
                            <><CreditCard className="w-4 h-4" aria-hidden="true" /> Pay ₹{(finalAmount || 0).toLocaleString("en-IN")} via Razorpay</>
                        )}
                    </button>

                    <p className="text-xs text-center text-gray-400">
                        🔒 Secured by Razorpay · SSL encrypted · DPDP Act 2023 compliant
                    </p>
                </div>
            )}
                            ...
                            return (
                                <div key={c.id} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
                                    ...
                                    <button
                                        ...
                                        aria-label={`Pay ₹1,000 to ${c.alias} via Razorpay`}
                                    ...
                                    >
                                        {campaignLoading === c.id ? (
                                            ...
                                            <><Heart className="w-4 h-4" /> Contribute ₹1,000 via Razorpay</>
                                        )}
                                    </button>
                                </div>
                            );
